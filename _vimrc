...
sil! set noshowmatch title noerrorbells splitright cmdwinheight=10 showcmd
...
" Comments
iab _cc <C-R>="// " . repeat("-", 76)<CR><CR>
iab _ct <C-R>="/" . repeat("/", 78)<CR><CR>
iab _cm ```c<CR>```<esc>O
...
function! ExecCompiler()
  ...
  if &ft == "prg"
    ...
    if strlen($WSL_DISTRO_NAME) > 0
      let l:file = substitute(l:file, "/mnt/c", "c:", "g")
      :exe "!jarvis_w32.exe" l:file "&"
      :redraw!
    elseif has("win32")
